   ...
   <!--          <a class="edit btn btn-primary" href="{{url('admincp/user/tao-user')}}">
           ...
           @foreach($articles as $value)
            <tbody>
               <tr>
                   <td>{{($value['ngay'])}}</td>
                   <td>
                       @foreach($value['ds_datphong'] as $dsdatphong)
                           <p>
                             {{$dsdatphong->loaiphong->sTenLP}} (số phòng: {{$dsdatphong->iSoluongphong}}) (số khách: {{$dsdatphong->iSoluongnguoi}})
                           </p>
                       @endforeach
                   </td>
                   <td>
                           @foreach($value['luongkhachhang'] as $lkh)
                             {{$lkh->luongnguoi}}
                           @endforeach
                   </td>
                   <td>{{($value['doanhthu'])}} 
           ...
           @endforeach
             <tr>
                ...
                <td></td>
                      <td>
                         <p style="color: blue; font-size: 20px;">Tổng lượng khách:{{$tongkhachhang}}</p>
                      </td>
